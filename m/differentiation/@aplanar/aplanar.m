        ...
        function obj = cos(g)
            ...
            function do_fifth_order()
                obj.dxxxx(1,ii,jj,kk,ll,mm)=-(...
                    ...
                    g.dxx(1,kk,mm)*g.dxxx(1,ii,jj,ll)+...
                    g.dx(1,kk)*g.dxxxx(1,ii,jj,ll,mm)+...
                    g.dxxxx(1,jj,kk,ll,mm)*g.dx(1,ii)+...
                    g.dxxx(1,jj,kk,mm)*g.dxx(1,ii,ll)+...
                    g.dxx(1,jj,kk)*g.dxxx(1,ii,ll,mm)+...
                    g.dxxx(1,jj,ll,mm)*g.dxx(1,ii,kk)+...
                    g.dxx(1,jj,ll)*g.dxxx(1,ii,kk,mm)+...
                    g.dxx(1,jj,mm)*g.dxxx(1,ii,kk,ll)+...
                    g.dx(1,jj)*g.dxxxx(1,ii,kk,ll,mm)+...
                    g.dxx(1,ll,mm)*g.dxxx(1,ii,jj,kk)+...
                    g.dx(1,ll)*g.dxxxx(1,ii,jj,kk,mm)+...
                    g.dx(1,mm)*g.dxxxx(1,ii,jj,kk,ll)...
                    )*f...
                    -(...
                    g.dxx(1,jj,kk)*g.dxx(1,ii,jj)+...
                    g.dx(1,kk)*g.dxxx(1,ii,jj,ll)+...
                    g.dxxx(1,jj,kk,ll)*g.dx(1,ii)+...
                    g.dxx(1,jj,kk)*g.dxx(1,ii,ll)+...
                    g.dxx(1,jj,ll)*g.dxx(1,ii,kk)+...
                    ...
                    -(...
                    g.dxx(1,kk,mm)*g.dxx(1,ii,jj)+...
                    g.dx(1,kk)*g.dxxx(1,ii,jj,mm)+...
                    g.dxxx(1,jj,kk,mm)*g.dx(1,ii)+...
                    g.dxx(1,jj,kk)*g.dxx(1,ii,mm)+...
                    g.dxx(1,jj,mm)*g.dx(1,ii,kk)+...
                    g.dx(1,jj)*g.dxxx(1,ii,jj,kk)...
                    )*obj.dx(1,ll)...
                    -(...
                    g.dx(1,kk)*g.dxx(1,ii,jj)+...
                    g.dxx(1,jj,kk)*g.dx(1,ii)+...
                    ...
                    -(...
                    g.dxxx(1,jj,ll,mm)*g.dx(1,ii)+...
                    g.dxx(1,jj,ll)*g.dx(1,ii,mm)+...
                    g.dxx(1,jj,mm)*g.dxx(1,ii,kk)+...
                    g.dx(1,jj)*g.dxxx(1,ii,kk,mm)...
                    )*obj.dx(1,kk)...
                    -(...
                    g.dxx(1,jj,ll)*g.dx(1,ii)+...
                    g.dx(1,jj)*g.dxx(1,ii,ll)...
                    ...
                    -(...
                    g.dxx(1,jj,mm)*g.dx(1,ii)+...
                    g.dx(1,jj)*g.dxx(1,ii,mm)...
                    )*obj.dxx(1,kk,ll)...
                    -g.dx(1,ii)*g.dx(1,jj)*obj.dxxx(1,kk,ll,mm)...
                    -sx*g.dxxxxx(1,ii,jj,kk,ll,mm);
            end
        end
        function obj = cosh(g)
            obj=.5*(exp(g)+exp(-g));
        ...
        function C=diff(func,active,inactive,order)
            ...
            function C=redesign(C)
                ...
                if io>1
                    ...
                    C=autofill_symmetry(C,bigset(1:io),proto,1,nwrt);
                end
            end
...
% ::
%
%   C=autofill_symmetry(C,bigset,proto,current_level,nvars)
%
% Inputs
% -------
%
...
if siz(1)>1
    ...
    for irow=1:siz(1)
        output{irow}=autofill_symmetry(reshape(C(irow,:),proto_size),...
            bigset,proto,current_level,nvars);
    end
    C=cell2mat(output);
else
    ...
    for ii=1:finish
        ...
        if current_level==order
            ...
            C=autofill_symmetry(C,bigset,proto,current_level+1,nvars);
        end
    end
end
