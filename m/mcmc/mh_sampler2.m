function [Results]=mh_sampler2(logf,lb,ub,options,mu,SIG_mom)
...
%      - **linres** [empty|function handle|{@(x,varargin)x}]: function that
%         transforms the parameters before storing them and inverses the
%         operation. useful for instance in the presence of linear
%         restrictions, a situation in which the number of parameters
%         optimized over is smaller than the number of estimated
%         parameters. The function handle should take at least two input
%         arguments:
%         - the first argument is the vector of parameter to transform
%         - the second argument is true or false. When true the parameters
%         are transformed and when false the inverse operation is performed.
%         This is useful only when the sampling procedure has to recover or
%         to proceed from an unsufficient number of samples.
%
...
defaults={ % arg_names -- defaults -- checks -- error_msg
    ...
    'linres',[],@(x)isempty(x)||(isa(x,'function_handle') && nargin(x)==2),'linres must be empty or a function handle taking at least two input arguments'
...
if is_save
    ...
    if options.recover
        ...
        if summary.nchains
            ...
            for ic=1:nchain
                % at recovering, we unstransform
                mu_{ic}=linres(mu(:,ic),false);
...
Results=do_results();
    ...
    function Results=do_results()
        ...
        if ~isempty(linres)
            % at storing, we transform
            bestx=linres(bestx,true);
            
            for ii=1:numel(pop_)
                
                pop_(ii).x=linres(pop_(ii).x,true);
                
            end
            
        end
        
        Results=struct('pop',pop_,...
