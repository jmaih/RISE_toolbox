...
%        axis with plot data
    ...
    for c, (cond, df_c) in enumerate(data.groupby(condition, sort=False)):
        ...
        varargin.setdefault('marker', '' if interpolate else 'o')
        ls = varargin.pop('ls', '-' if interpolate else '')
        varargin.setdefault('linestyle', ls)
        label = cond if legend else '_nolegend_'
        ax.plot(x, central_data, color=color, label=label, varargin{:})
	end
...
function []= _plot_ci_band(ax, x, ci, color, err_kws, varargin):
...
function []= _plot_ci_bars(ax, x, central_data, ci, color, err_kws, varargin):
...
function []= _plot_boot_traces(ax, x, boot_data, color, err_kws, **varargin):
...
function []= _plot_unit_traces(ax, x, data, ci, color, err_kws, **varargin):
...
function []= _plot_unit_points(ax, x, data, color, err_kws, **varargin):
...
function []= _plot_boot_kde(ax, x, boot_data, color, varargin):
    %Plot the kernal density estimate of the bootstrap distribution.%
    varargin.pop('data')
    _ts_kde(ax, x, boot_data, color, varargin{:})
end


function []= _plot_unit_kde(ax, x, data, color, varargin):
    %Plot the kernal density estimate over the sample.%
    _ts_kde(ax, x, data, color, varargin{:})
end


function []= _ts_kde(ax, x, data, color, **varargin):
