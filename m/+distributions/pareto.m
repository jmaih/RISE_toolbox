function varargout=pareto(lowerquantileORmean,upperquantileORstdev,prob,c,d)
% INTERNAL FUNCTION: pareto distribution
%
% Note:
...
if hyperparameter_mode
    ...
    % check the pareto restrictions % 0 < x < inf
...
bb=1+sqrt(1+(m/s)^2); % 1-sqrt(1+(m/s)^2) also a solution
aa=m*(bb-1)/bb;
...
lpdf=log(b)+b.*log(a)-(b+1).*log(theta);
target=theta>a & theta<inf;
lpdf(~target)=-inf;
end
