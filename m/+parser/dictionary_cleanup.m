...
dictionary=orderfields(dictionary);
    ...
    function cleanup_parameters()
        ...
        dictionary.parameters.is_auxiliary=sparse([parameters.is_auxiliary]);
        ...
        [ss_pos,orig_pos]=locate_steady_state_parameters();
        
        dictionary.parameters.ss_pos=ss_pos;
        
        dictionary.parameters.orig_pos=orig_pos;
        
        function [ss_pos,orig_pos]=locate_steady_state_parameters()
            
            par_list=dictionary.parameters.name;
            
            auxiliary=dictionary.parameters.is_auxiliary;
            
            subList=par_list(auxiliary);
            
            subList=subList(strncmp(subList,'ss_',3));
            
            % The steady state is located at the true parameters
            subList_true=strrep(subList,'ss_','');
            
            orig_pos=locate_variables(subList_true,par_list);
            
            ss_pos=locate_variables(subList,par_list);
