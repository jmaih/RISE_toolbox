...
% INTERNAL FUNCTION: parser for dsge models
%
% ::
%
...
% Now we can discard definitions

if dictionary.definitions_inserted
    
    dictionary.definitions=dictionary.definitions(1:0);
...
if dictionary.is_model_with_planner_objective
    ...
    utils_derivatives=dictionary.planner_system.utils_derivs.recleaned_derivs;
...
% but the definitions have already been substituted
%--------------------------------------------------------------
routines.steady_state_model=static.shadow_steady_state_model;
...
[~,...
    ...
    myifelseif,...
    sig_pos... Location of the perturbation parameter
...
[wrt,dictionary.v,...
    ...
    dictionary.lead_lag_incidence.before_solve,exo_nbr,sig_pos);
%----------------------
...
probs_times_dynamic=parser.burry_probabilities(dynamic.shadow_model,myifelseif);
...
probs_times_dynamic=utils.code.code2func(probs_times_dynamic);

% now we replace calls to if_elseif: this occurs if there are switching
% parameters or there are more than 1 markov chain
%------------------------------------------------------------------------
routines.probs_times_dynamic=probs_times_dynamic;

if numel(dictionary.chain_names)>1
    
    routines.probs_times_dynamic=parser.if_elseif_to_if_elseiff(routines.probs_times_dynamic);
...
if dictionary.is_model_with_planner_objective
    ...
    tmp=utils.code.code2func(planner_shadow_model);
    
    dictionary.planner_system.utils_derivs.derivatives(:,1)=...
        tmp(planobj_eq_nbr+(1:osr_derivs_eqn_nbr));
    ...
    Locs=osr_.derivatives(:,2);
    
    Locs=utils.code.set_vectorized_mapping(Locs);
    
    routines.planner_osr_support=struct('derivatives_re_order',der_reo,...
        'nwrt',osr_.nwrt,'size',osr_.size,...
        'second_order_util_derivatives',{osr_.derivatives(:,1)},...
        'derivatives_locations',Locs);
    ...
    %-------------------------------------------------
    routines.planner_loss_commitment_discount=tmp(1:planobj_eq_nbr);
    
    routines.planner_objective=tmp(1);
...
[dictionary.siz.after_solve,...
    ...
    ~isempty(sig_pos)); % whether the structural model has a perturbation parameter
...
v={
    ...
    'b_minus',0
    'e_0',0
    'sig_0',0 % perturbation parameter entering the structural model
    };
fields=v(:,1);
...
[siz,locations.t,locations.z,~,order_var,inv_order_var]=...
    ...
    LLI,...
    exo_nbr,... number of shocks
    logical(nsig));% number of shocks periods beyond the current
