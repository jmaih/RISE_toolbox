...
for ii=1:numel(equation_type)
    ...
    elseif is_planner
        ...
        dynamic.shadow_model=[dynamic.shadow_model;{sh_o}];
        static.shadow_model=[static.shadow_model;{sh_s}];
        static.shadow_BGP_model=[static.shadow_BGP_model;{sh_b1};{sh_b2}];
    end
end
% replace the steady state calls
%-------------------------------
dynamic.shadow_model=parser.replace_steady_state_call(dynamic.shadow_model);
static.shadow_model=parser.replace_steady_state_call(static.shadow_model);
static.shadow_BGP_model=parser.replace_steady_state_call(static.shadow_BGP_model);
