...
% Inflate parameters block with auxiliary switching parameters
%-------------------------------------------------------------
ppos=strcmp({blocks.name},'parameters');
blocks(ppos)=inflate_with_auxiliary_switching_parameters(blocks(ppos));
...
function blk=inflate_with_auxiliary_switching_parameters(blk)

pnames={blk.listing.name};

switch_names=pnames([blk.listing.is_switching]);
...
newblk=parser.listing();

proto=parser.listing('name','xxxx','is_in_use',true,'is_auxiliary',true);

for ii=1:numel(switch_names)
    
    v=switch_names{ii};
    
    [ssname,partname]=parser.create_auxiliary_param_specific_switch_param(v);
    
    newnames={ssname,partname};
    
    for jj=1:numel(newnames)
                
        proto.name=newnames{jj};
...
% then add the remaining auxiliary parameters controlling each type of
% perturbation
%----------------------------------------------------------------------
sig_iota1_iota2=parser.perturbation_control_param_names();

for jj=1:numel(sig_iota1_iota2)
    
    proto.name=sig_iota1_iota2{jj};
    
    newblk(end+1)=proto;
    
end

newnames={newblk.name};

bad=ismember(newnames,pnames);

if any(bad)
    
    disp(newnames(bad))
    
    error(['the parameter names above conflict with the auxiliary ',...
        'parameters created by RISE'])
    
end

blk.listing=[blk.listing(:).',newblk(:).'];
