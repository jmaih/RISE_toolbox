...
if ~dic.is_optimal_simple_rule_model
    ...
    for ieqtn=1:neqtns+1
        ...
        % skip the equation number
        %--------------------------
        locs=cell2mat(locs(2:end));
...
jac_toc=toc;
    ...
    function []=do_utility_differentiation()
        ...
        utils_derivs.recleaned_derivs=cleanup(utils_derivs.derivatives(:,1),true);
    ...
    function [x,old_size]=cleanup(x,remove_time)
        ...
        %-----------------------------------------------------------
        x=regexprep(x,'\<steady_state_(\w+)','steady_state($1)');
...
if isempty(leftpart)
    ...
    ekl=strfind(discount,'=');
    
    discount=discount(ekl+1:end-1);
