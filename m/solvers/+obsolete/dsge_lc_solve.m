function [T,R,SS,retcode,optim_opt,itercode,algo_name]=dsge_lc_solve(...
    ...
    shock_horizon,reordering_index,T0,optim_opt)
...
if lc_accelerate_solver
    ...
    [TT,RR,retcode,optim_opt,itercode,algo_name]=loose_commitment_solver(...
        ...
        W(dyn_cols(1:endo_nbr),dyn_cols(1:endo_nbr)),gam,betta,shock_horizon,[],[],optim_opt);
    ...
    if retcode
        ...
        if any(stat_cols)
            ...
            nregs=size(tmpBB,3);
            T=zeros(m);
            R=zeros(m,exo_nbr,shock_horizon,nregs);
        ...
        if debug
            ...
            [T_no_static,R_no_static]=loose_commitment_solver(...
                Aminus,A0,Aplus,B{:},W,gam,betta,shock_horizon,[],[],optim_opt);
            keyboard
        end
    end
    ...
    [T,R,retcode,optim_opt,itercode,algo_name]=loose_commitment_solver(...
        Aminus,A0,Aplus,tmpBB,W,gam,betta,shock_horizon,[],[],optim_opt);
end
