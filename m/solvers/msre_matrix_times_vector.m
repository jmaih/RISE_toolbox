    ...
    for st=1:h
        ...
        for jj=1:h
            ...
            g(iter_st)=g(iter_st)+Q(st,jj)*...
                utils.kronecker.A_kron_B_times_x(LMPRIME,Lplus(:,:,st),delta_vec(iter_jj),n,n);
%            g(iter_st)=g(iter_st)+Q(st,jj)*...
%                utils.kronecker.kron_times_vector(LMPRIME,Lplus(:,:,st),reshape(delta_vec(iter_jj),n,n));
        end
    end
else
    for st=1:h
        ...
        g(iter_st)=-delta_vec(iter_st)+...
            utils.kronecker.A_kron_B_times_x(transpose(Lminus(:,:,st)),Lplus(:,:,st),PD,n,n);
%        g(iter_st)=-delta_vec(iter_st)+...
%            utils.kronecker.kron_times_vector(transpose(Lminus(:,:,st)),Lplus(:,:,st),reshape(PD,n,n));
    end
end
...
% X=rand(n);
% for isim=1:Nsim
%     tic,T1=utils.kronecker.kron_times_vector(A,B,X);t0(1)=t0(1)+toc;
