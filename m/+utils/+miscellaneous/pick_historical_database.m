function histdb=pick_historical_database(m,dbtype,hist_end_date,filtration)
...
%
%   histdb=pick_historical_database(m,dbtype)
%
%   histdb=pick_historical_database(m,dbtype,hist_end_date)
%
% Args:
%
%    - **m** [rise|dsge] : model object
%
%    - **dbtype** [{'mean'}|'random'] : type of database. If 'mean', the
...
%    of history
%
%    - **filtration** [struct|empty] : if not empty the database is
%    collected from the structure. If empty, filtration is computed from
%    the model
%
% Returns:
%    :
...
%    ones.
%
%    - if the number of regimes is 1, "dbtype" and "hist_end_date" are
%    superfluous.
%
...
if isempty(filtration)
    
    [filtration,~,~,retcode]=filter(m);
...
if regimes_number==1||isempty(dbtype)
    
    dbtype='mean';
    
end

if ~any(strcmpi(dbtype,{'mean','random'}))
...
switch lower(dbtype)
    ...
    case 'mean'
        ...
        histdb=filtration.Expected_updated_variables;
        
        histdb=utils.miscellaneous.mergestructures(histdb,...
            filtration.updated_regime_probabilities);
        
    case 'random'
        
        reg_probs0=struct2pages(filtration.updated_regime_probabilities);
        ...
        for iname=1:numel(histnames)
            ...
            histdb.(name)=filtration.updated_variables.(name)(reg);
    ...
    otherwise
        ...
        error(['unknown type of database "',dbtype,'"'])
