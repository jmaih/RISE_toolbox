function [x,f,exitflag,obj]=studga_gate(PROBLEM)
%  STUDGA_GATE attempts to find the global minimum of a constrained function of
%  several variables.
%   STUDGA_GATE attempts to solve problems of the form:
%    min F(X)  subject to:  LB <= X <= UB   (bounds)
%     X
%
%   RES = STUDGA_GATE(FUN,X0,LB,UB) starts at X0 and finds a minimum X to the
...
%
%   [X,F,OBJ] = STUDGA_GATE(FUN,X0,LB,UB,OPTIONS) optimizes the function FUN under the
...
%     'max_fcount',inf);
%     x=studga_gate(@(x) FUN(x),x0,lb,ub,optimpot)
