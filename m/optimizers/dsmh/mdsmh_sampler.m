...
while ~done
    ...
    % do each core
    %--------------------
    for icore=1:G % use parallel here only if G>1% parfor (icore=1:G,G*(G>1)) % use parallel here only if G>1
        if common_moments
...
function pop=final_selection(pop,M,ndraws,minimization)
for istri=1:M
     pop{istri}=engine(pop{istri});
end
    function pop=engine(pop)
        ...
        lead=pop(1);
        X=[pop(2:end).x];
        dist=[inf,sqrt(sum(bsxfun(@minus,X,lead.x).^2,1))];
        [~,tags]=sort(dist);
        pop=sort_population(pop(tags(1:ndraws)));
    end
end
...
pop=cell(1,M);
offset=0;
binCounter=ndraws_per_striation*ones(1,M);
for ipop=1:M
    pop{ipop}=pop0(offset+(1:ndraws_per_striation));
    offset=offset+ndraws_per_striation;
end
...
metrop_draws=0;
metrop_accept=0;
median_guy=ceil(.5*ndraws_per_striation);
tic
for istri=1:M
%     theta_star=options.theta_star(istri);
    pointer=(istri-1)*ndraws_per_striation+median_guy;
    theta_star=pop0(pointer);
    for idraw=1:M_times_ndraws_per_striation
        theta=dsmh_draw();
        % beggar-thy-neighbor
        %--------------------
        % [st,striations,isnew]=find_striation(fx,striations,F,energy_adjust)
        
        destination=new_find_striation(theta.f);
        binCounter(destination)=binCounter(destination)+1;
        pop{destination}(binCounter(destination))=theta;
    end
    options.theta_star(istri)=theta_star;
end

% kill some of the parameter vectors to maintain the population in each
% striation constant
%--------------------------------------------------------------------------
pop=final_selection(pop,M,ndraws_per_striation,minimization);

% resort the database
%---------------------
pop=sort_population(cell2mat(pop));
...
%-------------------------
options.striations=create_striations([pop.f],M,options.energy_adjust);
...
if ~options.common_lambda && ~lambda_converged(lambda)
    ...
    function destination=new_find_striation(f)
        if f<F_pop(1)
            destination=1;
        elseif f>F_pop(end)
            destination=M;
        else
            [destination,striations,isnew]=find_striation(f,...
                striations,F_pop,options.energy_adjust);
            if isnew
                disp('found a new guy... this cannot happen')
            end
        end
    end
    ...
    function [theta]=dsmh_draw()
        ...
        function [theta,lgratio]=previous_level_draw()
            ...
            target_stri=new_find_striation(theta_star.f);
