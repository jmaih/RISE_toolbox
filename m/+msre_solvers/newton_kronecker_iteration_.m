            ...
            for jj=1:h
                ...
                if frwz
                    ...
                    g(iter_st)=g(iter_st)+Q(s_now,jj)*...
                        utils.kronecker.A_kron_B_times_x(LMPRIME,Lst(:,:,s_now)*Aplus(:,:,jj),delta_vec(iter_jj),nn,nn);
%%                    g(iter_st)=g(iter_st)+Q(s_now,jj)*...
%%                        utils.kronecker.kron_times_vector(LMPRIME,Lst(:,:,s_now)*Aplus(:,:,jj),reshape(delta_vec(iter_jj),nn,nn));
                end
            end
            if frwz
                g(iter_st)=-delta_vec(iter_st)+utils.kronecker.A_kron_B_times_x(LMPRIME,Lst(:,:,s_now)*Aplus(:,:,s_now),g(iter_st),nn,nn);
%%                g(iter_st)=-delta_vec(iter_st)+utils.kronecker.kron_times_vector(LMPRIME,Lst(:,:,s_now)*Aplus(:,:,s_now),reshape(g(iter_st),nn,nn));
            end
        end
    end
end
