function [iterating_function,...
    ...
    sampling_function] = fwz_newton_system(Aplus,A0,Aminus,Q)
...
% assignin('base','A',A)
% assignin('base','B',B)
% assignin('base','PSI',zeros(nn,1,hh))
% assignin('base','P',Q)
% assignin('base','s',ll)
% assignin('base','n',nn)
...
%             X(:,:,hh)= V*inv(AV(1:nn-ll,1:nn-ll));
    ...
    function [X,F]=X_update(X)
        ...
        X=X(:)-FPRIME\F(:);
        X=reshape(X,[ll,nn-ll,hh]);
        %X0=zeros(ll,(nn-ll)*hh);
        %for istate=1:hh
        %	icols=(istate-1)*(nn-ll)+1:istate*(nn-ll);
        %	X0(:,icols)=X(:,:,istate);
        %end
        %Xtest=reshape(X,ll,(nn-ll)*hh);
        %disp(max(max(max(abs(X0-Xtest)))))
    end
