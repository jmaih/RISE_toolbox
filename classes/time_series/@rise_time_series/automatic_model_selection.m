...
first=find(strcmp(start_date,tsdata.data(2:end,1)));
last=find(strcmp(end_date,tsdata.data(2:end,1)));
...
for iloop=1:number_of_loops
    ...
    function display_results(M,yname)
        ...
        disp([' The estimation sample is: ',tsdata.data{real_start+1,1},' -- ',tsdata.data{last+1,1}])
        if any(disabled)
            disp('=                                                               =')
