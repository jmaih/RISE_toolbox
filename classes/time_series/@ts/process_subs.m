...
switch nsubs
    ...
    case 1 % variables, dates or time shift
        ...
        elseif ischar(subs1)|| iscellstr(subs1) %#ok<*ISCLSTR> % dates or variable
    ...
    case {2,3} % dates and variables
        ...
        if nsubs==3 %and pages
            ...
            elseif isa(pages_,'function_handle')
                
                ok=true(size(pages));
                
                for iii=1:npages
                    
                    ok(iii)=all(pages_(datta(:,:,iii)));
                    
                end
                
                pages=pages(ok);
                
            elseif isnumeric(pages)
                
                pages=pages_;
                
            else
                
                error('wrong format for pages')
