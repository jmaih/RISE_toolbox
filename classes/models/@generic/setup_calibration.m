function obj=setup_calibration(obj,Calibration,is_forced)
% Set parameters
%
% ::
%
...
if ~is_forced
    ...
    forbidden=get(obj,'par_list(auxiliary)');
    
    bad=ismember(pnames,forbidden);
    
    if any(bad)
        
        disp(pnames(bad))
        
        error(['You cannot set the parameters above directly. Try changing ',...
            'the perturbation strategy instead'])
