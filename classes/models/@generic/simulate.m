...
nsols=Initcond.nsols;

retcode=zeros(1,nsols);

for isol=1:nsols
    
    [y,states,retcode(isol),Qt,myshocks]=utils.forecast.multi_step(y0(1),...
        Initcond.log_var_steady_state,Initcond.T(:,:,isol),...
        Initcond.state_vars_location,Initcond);
    
    if retcode(isol) && nsols==1
        % allow other solutions to simulate if they can
        
        break
        
    end
    
    [start_date,yy_isol,vnames]=one_simulation(y,Qt,myshocks,states);
    
    if isol==1
        
        yy=yy_isol(:,:,ones(nsols,1));
...
if obj.options.simul_to_time_series
    ...
    db={yy,...
        ...
        '3= number of solutions',nsols
        }
        };
end

    function [start_date,yy,vnames]=one_simulation(y,Qt,myshocks,states)
        ...
        [states_,markov_chains,nchains]=regimes2states(states,npy,smpl_states);
    ...
    function [states,markov_chains,nchains]=regimes2states(regimes_history,npy,smpl_states)
...
d={
    ...
    'simul_bgp_deviation(r)',false,@(x)islogical(x),...
