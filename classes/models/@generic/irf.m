...
parfor(ii=1:nobj,nworkers)
    
    myirfs{ii}=myintern(obj(ii));
        ...
        for istate=1:number_of_threads
            ...
            if ~retcode
                ...
                [xxxx,retcode]=utils.forecast.irf(y0,Initcond.T,...
                    ...
                    Initcond.state_vars_location,which_shocks,det_shocks,...
                    Initcond,user_input);
                ...
                if ~girf
                    ...
                    xxxx2=xxxx2-y0.y(relevant,ones(Initcond.nsteps+1,1),...
                        ones(nshocks,1),ones(irf_draws,1),ones(nsols,1));
...
%             Impulse_dsge(Initcond.is_log_var,:,:,:,:)=exp(Impulse_dsge(Initcond.is_log_var,:,:,:,:));
        ...
        % going from variables x time x shocks x irf_draws x regimes x sols
        % reshape as time x regimes x variables x shocks x irf_draws x sols
        %------------------------------------------------------------------
        Impulse_dsge=permute(Impulse_dsge,[2,5,1,3,4,6]);
            ...
            if irf_to_time_series
                ...
                for ishock=1:nshocks
                    ...
                    for vv=1:numel(irf_var_list)
                        ...
                        data=squeeze(Impulse_dsge(:,:,vlocs(vv),ishock,:));
                        
                        if ishock==1 && vv==1
                            
                            tmp=ts(startdate,data,RegimeNames);
                            
                        else
                            
                            tmp=set(tmp,'data',data);
