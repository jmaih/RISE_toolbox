...
if strcmp(type,'parameters')
    tmp.is_switching=false(1,n);
    tmp.is_trans_prob=false(1,n);
    tmp.governing_chain=ones(1,n);
    % re-tag the transition probabilities
    %------------------------------------
    for iparam=1:n
        tmp.is_trans_prob(iparam)=parser.is_transition_probability(tmp.name{iparam});
    end
elseif strcmp(type,'endogenous')
    % do nothing
