...
if add_shocks_correlations && ~isempty(obj.filtering)
    for imod=1:numel(model)
        shock_corr=corrcoef(ts.collect(model(imod).filtering.Expected_smoothed_shocks));
        mytable=[
            [' ',shock_list]
...
if add_shocks_correlations && ~isempty(obj.filtering)
    for imod=1:numel(obj)
        shock_corr=corrcoef(ts.collect(obj(imod).filtering.Expected_smoothed_shocks));
        mytable=[
            [' ',shock_list]
            shock_list',num2cell(shock_corr)
            ];
