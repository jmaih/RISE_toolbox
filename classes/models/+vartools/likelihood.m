...
for isol=1:ns
    ...
    [outCell{1:nargout}]=main_engine(sol(isol),M(:,:,isol),mapping,...
        y,x,is_time_varying_trans_prob,markov_chains,retcode(isol),regimes);
...
function [LogLik,Incr,retcode,filtering]=main_engine(sol,M,mapping,...
    y,x,is_time_varying_trans_prob,markov_chains,retcode,fixed_regimes)
...
is_known_regimes=~isempty(fixed_regimes);
...
filtering.Expected_smoothed_shocks=store_item(exo_names,...
    ...
    function [Incr,ut,prob_udate,prob_filt]=msvar_likelihood(B,SIG)
        ...
        if is_known_regimes
            ...
            if numel(fixed_regimes)~=T
                
                error('the number of regimes must be equal to the number of observations')
                
            end
            
            if ~all(ismember(fixed_regimes,1:h))
                
                error(['all regimes must be in [1,',int2str(h),']'])
                
            end
            
            for t=1:T
                
                prob_filt(fixed_regimes(t),t)=1;
                
            end
            
            prob_udate=prob_filt(:,1:T);
