...
algotype=@(x)functype(x)||iscell(x);
    ...
    mydefaults={
        ...
        'n x 3 cell {endogNames,paramNames,endogValues}']
        ...
        'solve_shock_horizon(r)',[],@(x)num_fin_int(x),'solve_shock_horizon must be >=0'
        ...
        (iscell(x) && isa(x{2},'function_handle') && (num_fin_int(x{1}) && x{1}>0))||...
        (num_fin_int(x) && x>0),...
        ['solve_occbin must be a positive integer or a two-element cell ',...
        'whose first element is a positive integer and the second a ',...
        'function handle']
