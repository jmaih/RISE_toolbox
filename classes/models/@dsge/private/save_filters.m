function filtering=save_filters(obj,filtering)
% INTERNAL FUNCTION
%

Fields=fieldnames(filtering);
...
for ifield=1:numel(Fields)
    ...
    if isfield(filtering,ff)
    
        filtering.(ff)=push_filter_to_time_series(filtering.(ff),ff);
...
filtering.rolling_multistep=struct();
...
if nsteps>1
    ...
    for ivar_=1:nvars
        ...
        thisUpdate=double(filtering.smoothed_variables.(vnames{ivar_}));
        
        thisExpectedUpdate=double(filtering.Expected_smoothed_variables.(vnames{ivar_}));
        
        for ireg=1:h
            
            rolling_batch=[thisUpdate(:,ireg),rolling{ireg}(2:end,:,ivar_)];
            
            filtering.rolling_multistep.filtered_variables.(['regime_',sprintf('%0.0f',ireg)]).(vnames{ivar_})=...
        ...
        filtering.rolling_multistep.Expected_filtered_variables.(vnames{ivar_})=...
            ts(obj.dates_smoothing,rolling_batch,mynames);
    ...
    filtering.rolling_multistep.rmse=ts(1,permute(rmse,[2,1,3]),vnames);

    filtering.rolling_multistep.Expected_rmse=ts(1,permute(expect_rmse,[2,1]),vnames);
    ...
    function out=push_filter_to_time_series(filtdata,filtname)
        ...
        variables_flag=parser.mycontains(filtname,'_variables');
        
        shocks_flag=parser.mycontains(filtname,'_shocks');
        
        measerrs_flag=parser.mycontains(filtname,'_measurement_errors');
        
        filtered_flag=parser.mycontains(filtname,'filtered_');
        
        regimes_flag=parser.mycontains(filtname,'_regime_');
