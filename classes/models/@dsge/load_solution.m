function [T,R,Qfunc,steady_state,new_order,state_vars_location,log_vars]=...
...
T=cell(order,regimes_number,nsols);

R=cell(1,regimes_number,nsols);
...
% alphabetical order (inv_order_var)
%------------------------------------
...
if do_bvar_dsge
    ...
    function inv_order_var_solution()
        ...
        for isol=1:nsols
            ...
            for ireg=1:regimes_number
                ...
                tmp(:,t_pb)=T{1,ireg,isol}(:,z_pb);
                
                % separate autoregressive part from shocks
                %-----------------------------------------
                Tz{1,ireg,isol}=tmp(iov,iov);
                
                R{1,ireg,isol}=T{1,ireg,isol}(iov,e_0(1):end);
                
                if regimes_number>1
                    
                    if ireg==1 && isol==1
                        
                        npges=size(R{1,ireg,isol},2)/exo_nbr_;
                        
                    end
                    
                    R{1,ireg,isol}=reshape(full(R{1,ireg,isol}),...
                        [endo_nbr_,exo_nbr_,npges]);
    ...
    function order_var_solution()
        ...
        % The transition matrix is evaluated alphabetically!!!
        Qfunc=small_memo(Qfunc,iov,log_vars(ov));
        ...
        for io=1:order
            ...
            for ireg=1:regimes_number
                ...
                for isol=1:nsols
                    ...
                    T{io,ireg,isol}=obj.solution.(['T',zzz(1:io)]){1,ireg,isol}(ov,:);
...
function out=small_memo(Qfunc,iov,ov_log_vars)

out=@engine;

    function varargout=engine(x)
        % log_vars is in order_var order
        x(ov_log_vars)=exp(x(ov_log_vars));
        
        % Qfunc needs the iov (or alphabetical) order
        x=x(iov);
        
        [varargout{1:nargout}]=Qfunc(x);
