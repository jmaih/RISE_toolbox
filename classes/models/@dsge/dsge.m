classdef dsge < generic % & gogetter
    ...
    properties (Dependent,Hidden)
        % log vars in alphabetical order
        log_vars
    ...
    properties (Constant, Hidden = true)
        ...
        % Subfolders
        %-----------
        subfolders_list={'routines'};%'graphs','estimation','simulations',
    ...
    methods
        ...
        function obj=dsge(model_filename,varargin)
            ...
            disp([mfilename,':: ',switch_type,' Switching ',model_type,...
                ' model in ',int2str(nregs),' regimes detected'])
        ...
        function n=get.nsols(obj)
            
            n=0;
            
            if ~isempty(obj.solution) && isfield(obj.solution,'Tz')
                
                n=size(obj.solution.Tz,3);
        ...
        varargout=accuracy(varargin)
        
        varargout=bvar_dsge(varargin)
        
        varargout=check_derivatives(varargin)
        
        varargout=compute_steady_state(varargin)
        
        varargout=counterfactual(varargin)
    ...
    methods(Hidden=true)
        ...
        varargout=set_auxiliary_switching_parameters(varargin)
