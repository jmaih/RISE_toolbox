...
simul_recursive=obj.options.simul_recursive;

forecast_exogenous=obj.options.simul_exogenous_func;
if isempty(forecast_exogenous)
    error('an simulation function for the exogenous should be provided in simul_exogenous_func')
end
...
if ~retcode
    ...
    jacobian_dynamic_model=obj.routines.probs_times_dynamic_derivatives(1);
    ...
    function [Y,retcode]=sparse_simulation_algorithm(Y)
        ...
        function [Y,crit]=iteration_engine(Y)
            ...
            crit=max(abs(dx)); % <---crit=max(abs(resid(:)));
                Y(order_var,2:end-1)=Y(order_var,2:end-1)+reshape(dx,endo_nbr,[]);
        end
    end
