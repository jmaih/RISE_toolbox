function [init,retcode,nsols]=filter_initialization(obj,varargin)
...
if nsols>1
    ...
    for isol=1:nsols
        ...
        for io=1:order
            ...
            sols_i.(tease)=sols_i.(tease)(:,:,isol);
            
        end
        
        obji=obj;
        
        obji.solution=sols_i;
        
        [init{isol},retcode(isol)]=filter_initialization(obji);
...
%--------------------------------------------------------------------------
    function [init,retcode]=initialize_filter()
        ...
        %------------------------------------------------------------------
        [T,~,Qfunc,steady_state,new_order,state_vars_location]=load_solution(obj,'ov');
        ...
        % Qfunc now reorders variables itself, no more need to input iov
        Qfunc=small_memo(Qfunc,1:numel(iov),grand_order_var_state);
        ...
        if PAI00_given
            ...
            [Q]=evaluate_filter_transition_matrix(Qfunc,steady_state,1/h*ones(1,h)); %<---Q=Qfunc(steady_state{1});
