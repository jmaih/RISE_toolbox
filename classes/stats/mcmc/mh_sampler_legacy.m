...
%       structure with fields "every", "location", "filename". A finite
%       'every' triggers the saving to disk!
%       - **recover** [false|{true}]: attempts to recover from an earlier
%       aborted simulation. If the simulation is aborted with the option of
%       not saving to disk, the draws are lost forever. But if it is simply
%       a problem of convergence, one can recover/continue by setting mu in
%       the arguments list to the output of the previous run. If the draws
%       are saved to disk, then provided that the location of the stored
%       draws is passed as an option, RISE will continue from the abortion.
% 
%     - **mu** [d x 1 vector | struct]: initial condition for the sampler or
%     previous simulation structure in case one wants to continue the
%     previous run.
% 
