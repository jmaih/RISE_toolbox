function [Jac,auxiliary]=jacobian(objectives,varnames,wrt,out_type)
% objectives is a function handle or an array of function handles
% wrt is a sad variable or a cell array of such.
% output is returned in a sad form in case the user wants to use it to
% recompute the hessian, which would be much easier to compute from the
% sad form
if nargin<4
    out_type='char'; %['expand','sad']
    if nargin<3
        wrt=[];
        if nargin<2
...
args=repmat({sad('xxx')},numel(args),1);
...
for irow=1:nrows
    ...
    for icol=1:numel(wrt)
        ...
        if any(target)
            ...
            if strcmp(out_type,'char')
                Jac{irow,icol}=char(Jac{irow,icol});
            elseif strcmp(out_type,'expand')
                Jac{irow,icol}=char(Jac{irow,icol},true);
            end
        end
    end
end
...
auxiliary={};
if nargout>1 && strcmp(out_type,'char')
    [Jac,auxiliary]=sad.trim(Jac);
    % destroy mapObj_sad from the workspace...
end
end
