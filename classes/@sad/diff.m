...
% sad/DIFF overloads diff with a sad object argument
% DIFF(S,'v')
% DIFF(S,'v1','v2',...,'vn')
...
for ider=1:nder
    ...
    if all(~isnan(locs)) % all variables must be present else the derivative is zero
        ...
        for iloc=1:numel(locs)
            ...
            if iloc<numel(locs)
                % push all the temporary variables inside and delete the
                % guy in the workspace
                func_ider=str2func([func_declare,derivs{ider}]);
            end
        end
        % collect the temporary variables alongside the derivatives...
