classdef mcf < handle
    ...
    methods
        ...
        function obj=mcf(check_behavior,nsim_or_draws,lb,ub,names,procedure_)
            ...
            %          those extra output arguments while sampling.
            %      (ii) when it is a cell, it should be a two-element cell such that the first
            %           element is the function handle and the second is the number
            %           of outputs desired by the user.
        ...
        function hdl=correlation_patterns_plot(obj,names,type,pval_cutoff)
            ...
            title=sprintf('%s :: Correlation patterns in the %s sample',mfilename,type);

            hdl=figure('name',title);
        ...
        function hdl=scatter(obj,names,type,pval_cutoff,title)
            ...
            %   hdl=scatter(obj,names,type,pval_cutoff,title)
            %
            % Args:
            %
            %    - **obj** [mcf object]:
            ...
            if isempty(title)
                ...
                title=sprintf('%s :: scatter plot of the data in %s the sample',mfilename,type);
            ...
            if any(good)
                ...
                hdl=utils.plot.multiple(@(xname)do_one_scatter(xname),...
                    paired_names,title,obj.graph_nrows,obj.graph_ncols,...
                    'FontSize',11,'FontWeight','normal');
    ...
    methods (Static)
        ...
        function [pValue,Dn,xn,largest]=kolmogorov_smirnov_test(d)
            ...
            % ::
            %
            %    [pValue,Dn,xn,largest]=kolmogorov_smirnov_test(d)
            %
            % Args:
            %    d : mcf object
            %
