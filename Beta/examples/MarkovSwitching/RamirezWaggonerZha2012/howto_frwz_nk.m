...
for imod=1:numel(Models)
    ...
    if isempty(alphakrol)
        ...
        generic{imod}=set_parameters(generic{imod},eval(mm(alphakrol+1:end))); 
    end
end
...
bk2=set_parameters(bk,cal_2); 
zer2=set_parameters(zer,cal_2);
for isol=1:numel(solvers)
    disp([' =============== ',solvers{isol},' =============== '])
...
for isol=1:numel(solvers)
    ...
    newmod=set_parameters(newmod,cal_2);
