...
if forecast_param_uncert
    ...
    if ~posterior_density_ready
        mode_density_ready=~isempty(vertcat(obj.estimated_parameters.mode));
        type='mode_density';
    ...
    if isempty(parameters)
        parameters=vertcat(obj.estimated_parameters.mode); % 'mode'
        if isempty(parameters)
            parameters=vertcat(obj.estimated_parameters.startval); % 'calibration'
        end
    end
end
...
if isempty(forecast_start_date)
    smpl_end_date=forecast_historical_db.TimeInfo(end);
    forecast_start_date=smpl_end_date+1;
else
    forecast_start_date=rise_date(forecast_start_date);
end
if isempty(forecast_end_date)
    smpl_end_date=forecast_historical_db.TimeInfo(end);
    forecast_end_date=smpl_end_date+12;
else
    forecast_end_date=rise_date(forecast_end_date);
end
...
model_varnames={obj.varendo.name};
...
if ~incomplete_state
    ...
    if incomplete_state
        ...
        start_vector=start_vector(:,ones(1,obj.NumberOfRegimes));
    end
end
...
if forecast_regime_uncert 
    ...
    if ~ismember(obj.options.forecast_regime,(1:obj.NumberOfRegimes))
        error([mfilename,':: Chosen regime must be in [1,',int2str(obj.NumberOfRegimes),']'])
    end
    if ~isempty(forecast_conditional_info)
        endo_vars={};
        exo_vars={};
        ...
        for ii=1:numel(fields)
            if ismember(fields{ii},{'lower_bound','upper_bound','central_tendency'})
                ...
                for oo=1:new_ts.NumberOfVariables
                    ...
                    if ismember(vo,{obj.varendo.name})
        ...
        restrictions_id=locate_variables(endo_vars,{obj.varendo.name});
...
history=1;
cond_fkst=nan(obj.NumberOfEndogenous(2),history+nsteps,forecast_paths);
cond_fkst_mean=nan(obj.NumberOfEndogenous(2),history+nsteps,forecast_paths);
uncond_fkst=nan(obj.NumberOfEndogenous(2),history+nsteps,forecast_paths);
...
while ii<forecast_paths
    ...
    if forecast_param_uncert
        ...
        if ii==0 % solve the model only once
            [obj,retcode]=solve(obj,'evaluate_params',parameters);
            if retcode
...
cond_fkst_db=rise_time_series(forecast_start_date-1,permute(cond_fkst,[2,1,3]),{obj.varendo.name});
cond_fkst_mean_db=rise_time_series(forecast_start_date-1,permute(cond_fkst_mean,[2,1,3]),{obj.varendo.name});
uncond_fkst_db=rise_time_series(forecast_start_date-1,permute(uncond_fkst,[2,1,3]),{obj.varendo.name});

% obj.forecasts=rise_time_series(obj.options.data.start,permute(y,[]),char({obj.varendo.name}));
...
%
    ...
    function [mr_cond_fkst,mr_cond_fkst_mean,mr_uncond_fkst]=multi_regime_forecast()
        PAI_F=[PAI,nan(obj.NumberOfRegimes,nsteps)];
        mr_uncond_fkst=nan(obj.NumberOfEndogenous(2),nsteps+1,obj.NumberOfRegimes);
        mr_cond_fkst_mean=mr_uncond_fkst;
        for h=1:obj.NumberOfRegimes
            prev=h;
            ...
            for t=1:nsteps
                ...
                if forecast_shock_uncert
                    mr_uncond_fkst(:,t+1,h)=mr_uncond_fkst(:,t+1,h)+obj.R(:,:,st)*randn(obj.NumberOfExogenous,1);
                end
                prev=st;
            end
        end
        ...
        switch model_has_to_be_solved+2*state_has_to_be_computed
            ...
            case {2,3}
                ...
                if ~retcode
                    ...
                    PAI=nan(obj.NumberOfRegimes,1);
                    for kk=1:obj.NumberOfRegimes
                        PAI(kk)=1;
                    ...
                    %
                    y0=nan(obj.NumberOfEndogenous(2),obj.NumberOfRegimes);
                    for kk=1:obj.NumberOfEndogenous(2)
