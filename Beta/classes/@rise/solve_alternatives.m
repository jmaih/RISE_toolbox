...
solve_alternatives_nsim=100;
if isempty(obj)
    if nargout>1
        error([mfilename,':: when the object is emtpy, nargout must be at most 1'])
    end
    allobj=struct('solve_alternatives_nsim',solve_alternatives_nsim,...
...
mode=vertcat(obj.estimated_parameters.mode);
[obj,retcode]=obj.solve('evaluate_params',mode);
sols=0;
indep_sols=0;
if ~retcode
    sols=sols+1;
...
for sim=1:solve_alternatives_nsim-1
    [objtmp,retcode]=obj.solve('evaluate_params',mode);
    if ~retcode
        sols=sols+1;
        do_not_discard=true;
        for jj=1:indep_sols
