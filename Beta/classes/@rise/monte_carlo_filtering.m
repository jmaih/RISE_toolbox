...
% r0=obj.options.graphics(1); % default number of rows in figure
% c0=obj.options.graphics(2);% default number of columns in figure

%% get the data for likelihood computation
smooth_flag=false;
if obj.NumberOfObservables(1)>0 % || isempty(obj.varobs)
    % load the data
    [obj,junk,retcode]=load_data(obj);
    if retcode
        warning([mfilename,':: ',decipher_error(retcode)]) %#ok<WNTAG>
    else
...
bounds=[vertcat(obj.estimated_parameters.lb),vertcat(obj.estimated_parameters.ub)];
npar=size(bounds,1);
...
for isim=1:monte_carlo_nsim
    [obj,retcode(isim)]=solve(obj,'evaluate_params',xparam(:,isim));
