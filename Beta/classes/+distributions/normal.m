function varargout=normal(plb,pub,prob,c,d)
...
if hyperparameter_mode
    ...
    % check the normal restrictions % x finite
    if ~isfinite(plb)
        error([mfilename,':: plb must be positive and finite'])
    end
    if ~isfinite(pub)
        error([mfilename,':: pub must be positive and finite'])
    end
    if pub<=plb
...
aa=m;
bb=s;
ab=fsolve(@objective,[aa;bb],optimset('display','iter'),c,d,m,s);
a=ab(1);
b=ab(2);
...
lpdf=-log(b*sqrt(2*pi))-.5*((theta-a)./b).^2;
