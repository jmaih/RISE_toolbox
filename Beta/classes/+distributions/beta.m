function varargout=beta(plb,pub,prob,c,d)
...
if hyperparameter_mode
    ...
    if plb<c||plb>d
        error([mfilename,'plb must satisfy  c < x < d'])
    end
    if pub<c||pub>d
        error([mfilename,'pub must satisfy  c < x < d'])
    end
    if pub<=plb
...
aa=cmmd*bb;
ab=fsolve(@objective,[aa;bb],optimset('display','iter','tolfun',sqrt(eps)),c,d,m,s);
a=ab(1);
b=ab(2);
