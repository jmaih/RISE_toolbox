function varargout=uniform(plb,pub,prob,c,d)
...
if hyperparameter_mode
    ...
    if ~isfinite(plb)
        error([mfilename,'plb must satisfy  -inf < x < inf'])
    end
    if ~isfinite(pub)
        error([mfilename,'pub must satisfy  -inf < x < inf'])
    end
    if pub<=plb
    ...
    function [violation,space]=hyperparameters(hyper)
        space=[-inf,plb; % bounds for first hyperparameter
            pub,inf]; % bounds for second hyperparameter
