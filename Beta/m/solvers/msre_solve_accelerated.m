...
if isempty(T0)
    ...
    switch model_class
        ...
        case 3 % hybrid model
            ...
            %                 [~,~,~,sampling_function] = msre_solvers.fwz_newton_system(Aplus,A0,Aminus,Q);
            %             end
...
[T,R,gsig,retcode,itercode,algo_name]=...
    ...
    function [T,R,gsig,retcode,itercode,algo_name]=...
        ...
        fwz_flag=false;
        [iterate_func,regimes,regimes_red,is_reduced]=reduced_problem();
        ...
        if impose_solution
            ...
            if isevs
                ...
                if fwz_flag % transform T into X
                    T0=fwz_inverse_solution_func(T0);
                end
                ...
                if fwz_flag && ~retcode % transform X into T
                    T=fwz_solution_func(T);
                end
                if is_reduced && ~retcode
                    % then build back the entire solution
                    Ttmp=nan(nn,nn,h);
                    for i_state=1:h
                        Ttmp(:,:,i_state)=T(:,:,regimes(i_state)==regimes_red);
                    end
                    T=Ttmp;
                    clear Ttmp;
                end
        ...
        function [iterate_func,regimes,regimes_red,is_reduced]=reduced_problem()
            ...
            for istate=1:h
                if istate>1
                    ...
                    for jstate=1:istate-1
                        if max(max(abs(A0(:,:,jstate)-A0(:,:,istate))))<1e-10
                            if max(max(abs(Aplus(:,:,jstate)-Aplus(:,:,istate))))<1e-10
                                if max(max(abs(Aminus(:,:,jstate)-Aminus(:,:,istate))))<1e-10
                                    alone=false;
            ...
            switch solver
                case {0,'msre_klein'}
                    iterate_func = @(x)msre_klein(x,Aplus(:,:,regimes_red),A0(:,:,regimes_red),Aminus(:,:,regimes_red),Q_red,nn,h_red);
                case {1,'msre_gensys'}
                    iterate_func = @(x)msre_gensys(x,Aplus(:,:,regimes_red),A0(:,:,regimes_red),Aminus(:,:,regimes_red),Q_red,nn,h_red);
                case {2,'msre_aim'}
                    iterate_func = @(x)msre_aim(x,Aplus(:,:,regimes_red),A0(:,:,regimes_red),Aminus(:,:,regimes_red),Q_red,nn,h_red);
                case {3,'functional_iteration'}
                    iterate_func = @(x)msre_solvers.functional_iteration_(x,Aplus(:,:,regimes_red),A0(:,:,regimes_red),Aminus(:,:,regimes_red),Q_red,nn,h_red,solve_fwz_hypothesis);
                case {33,'functional_iteration_'}
                    iterate_func = @(x)msre_solvers.functional_iteration(x,Aplus(:,:,regimes_red),A0(:,:,regimes_red),Aminus(:,:,regimes_red),Q_red,nn,h_red,solve_fwz_hypothesis);
                case {4,'newton_kronecker'}
                    iterate_func = @(x)msre_solvers.newton_kronecker(x,Aplus(:,:,regimes_red),A0(:,:,regimes_red),Aminus(:,:,regimes_red),Q_red,nn,h_red,solve_fwz_hypothesis);
                case {44,'newton_kronecker_'}
                    iterate_func = @(x)msre_solvers.newton_kronecker_(x,Aplus(:,:,regimes_red),A0(:,:,regimes_red),Aminus(:,:,regimes_red),Q_red,nn,h_red,solve_fwz_hypothesis);
                case {5,'newton_system'}
                    iterate_func = @(x)msre_solvers.newton_system(x,Aplus(:,:,regimes_red),A0(:,:,regimes_red),Aminus(:,:,regimes_red),Q_red,nn,h_red,solve_fwz_hypothesis);
                case {55,'newton_system_'}
                    iterate_func = @(x)msre_solvers.newton_system_(x,Aplus(:,:,regimes_red),A0(:,:,regimes_red),Aminus(:,:,regimes_red),Q_red,nn,h_red,solve_fwz_hypothesis);
                case {6,'newton_kronecker_iteration'}
                    iterate_func = @(x)msre_solvers.newton_kronecker_iteration(x,Aplus(:,:,regimes_red),A0(:,:,regimes_red),Aminus(:,:,regimes_red),Q_red,nn,h_red,solve_fwz_hypothesis);
                case {66,'newton_kronecker_iteration_'}
                    iterate_func = @(x)msre_solvers.newton_kronecker_iteration_(x,Aplus(:,:,regimes_red),A0(:,:,regimes_red),Aminus(:,:,regimes_red),Q_red,nn,h_red,solve_fwz_hypothesis);
                case {7,'fwz_newton_system'}
                    [iterate_func,fwz_solution_func,fwz_inverse_solution_func] = msre_solvers.fwz_newton_system(Aplus(:,:,regimes_red),A0(:,:,regimes_red),Aminus(:,:,regimes_red),Q_red);
                    fwz_flag=true;
                otherwise	% @(x)msre_solvers.(mysolvers{isol})(x,Aplus(:,:,regimes_red),A0(:,:,regimes_red),Aminus(:,:,regimes_red),Q_red,nn,h_red,solve_fwz_hypothesis);
                    if isnumeric(solver)
                    ...
                    elseif ischar(solver)
                        ...
                        iterate_func = @(x)iterate_func(x,Aplus(:,:,regimes_red),A0(:,:,regimes_red),Aminus(:,:,regimes_red),Q_red);
                    elseif  isa(solver,'function_handle')
                        iterate_func = @(x)solver(x,Aplus(:,:,regimes_red),A0(:,:,regimes_red),Aminus(:,:,regimes_red),Q_red);
                    else
