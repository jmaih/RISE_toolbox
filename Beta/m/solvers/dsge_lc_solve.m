function [T,R,SS,retcode,optim_opt,itercode,algo_name]=dsge_lc_solve(...
    ...
    order,reordering_index,T0,optim_opt)
...
if lc_accelerate_solver
    ...
    [TT,RR,retcode,optim_opt,itercode,algo_name]=loose_commitment_solver(...
        ...
        AAplus(static_endo_nbr+1:end,static_endo_nbr+1:end),BB(static_endo_nbr+1:end,:),...
        W(dyn_cols(1:endo_nbr),dyn_cols(1:endo_nbr)),gam,betta,order,[],[],optim_opt);
    ...
    if retcode
        ...
        if any(stat_cols)
            ...
            Bbar_1=BB(1:static_endo_nbr,:);
            clear AA0 AAminus AAplus BB
            
            % initialize solution matrices
            T=zeros(m);
            R=zeros(m,exo_nbr,order);
            ...
            R(xs,:,1)=-iAbar0_11*(C*R(xd,:,1)+Abar_plus_12*T(xd,mu2)*R(mu2,:,1)+Bbar_1);
            for ii=2:order
                R(xs,:,ii)=-iAbar0_11*(C*R(xd,:,ii)+Abar_plus_12*T(xd,mu2)*R(mu2,:,ii)+Abar_plus_12*R(xd,:,ii-1));
            end
        ...
        if debug
            ...
            [T_no_static,R_no_static]=loose_commitment_solver(...
                Aminus,A0,Aplus,B,W,gam,betta,order,[],[],optim_opt);
            keyboard
        end
    end
