...
if init
    ...
    restricted_list={'Contents','rise','prior_plots','initialize_solution_or_structure'};
    ...
    expansion_order=myoptions.solve_expect_order;
    % shocks are allowed to be anticipated.
    shock_properties=struct('name',{obj.exogenous.name},...
        'StandardDeviation',num2cell(nan(sum(obj.exogenous.number),1)),...
        'horizon',num2cell(expansion_order*ones(sum(obj.exogenous.number),1)));
    rows=4;cols=3;
    MainOptions={
    ...
    if nargs>0
        ...
        for ii=1:nargs/2
            ...
            if strcmp(propname,'estim_priors')
...
% % %                 assert(isa(propval,'rise_time_series'),'data input must be a rise_time_series object')
            ...
            if strcmp(propname,'shock_properties')
                ...
                for jj=1:nshocks
                    ...
                    % update the solve_expect_order if necessary
                    obj.options.solve_expect_order=max(propval(jj).horizon,obj.options.solve_expect_order);
                end
                continue
            end
