...
if retcode && obj.options.debug
    ...
    function retcode=solve_zeroth_order()
        ...
        if resolve_it
            ...
            if ~retcode
                ...
                ss_=obj.solution.ss{1};
                ys_=ss_;
                x=zeros(sum(obj.exogenous.number),1);
                [obj.solution.Q,retcode]= online_function_evaluator(obj.func_handles.transition_matrix,ys_,x,ss_,params(:,1),def_,s0,s1);
                % measurement errors
                %-------------------
                if ~retcode
    ...
    function retcode=solve_first_order()
        ...
        [obj.G1,retcode]=evaluate_structural_derivatives(derivatives);
%         retcode=evaluate_derivatives();
        ...
        if resolve_it
            % May be necessary to compute planner information first
            %------------------------------------------------------------
    ...
    function retcode=solve_second_order()
        ...
        %----------------------------------
        derivatives=obj.model_derivatives.Endogenous_Shocks_Parameters(2);
        [obj.G2,retcode]=evaluate_structural_derivatives(derivatives);
        if retcode
            return
        end
    ...
    function [D,retcode]=evaluate_structural_derivatives(derivatives)
        ...
        for s0=1:h
            for s1=1:h
                ...
                D{s0,s1}=online_function_evaluator(derivatives,ys(:,s0),xss,ss(:,s0),params(:,s0),def{s0},s0,s1);
                if ~valid(D{s0,s1})
...
%         obj.G1=cell(h);
%         retcode=0;
%         % first derivatives with respect to endogenous, exogenous and
...
%                 switch derivative_type
%                     case 'numerical'
%                         obj.G1{s0,s1}=my_numerical_derivatives(derivatives,ys(:,s0),xss,ss(:,s0),params(:,s0),def{s0},s0,s1);
%                     case 'automatic'
%                         obj.G1{s0,s1}=my_automatic_derivatives(derivatives,ys(:,s0),xss,ss(:,s0),params(:,s0),def{s0},s0,s1);
%                 end
