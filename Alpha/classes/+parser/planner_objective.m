...
if ~isempty(startc)
    ...
    if ~ismember(true_commitment,{'0','1'})
        dictionary.markov_chains(end+1)=struct('name','loosecommit','number_of_states',2,'is_endogenous',false);
        % Adding a new markov chain through loose commitment. We need to
        % re-order everything before proceeding. 
