classdef planar
    ...
    methods(Static)
        function deriv=derivatives2functions(deriv,args,compact,optimize)
            ...
            %--------------------------
            deriv.derivatives=analytical_symbolic_form(deriv.derivatives,args,'analytic');
        ...
        function [derivs,DerivPartitions]=differentiate(eqtns,order,partitions,verbose,early_intercept)
            ...
            for oo=1:order
                ...
                % build the grids
                %----------------
                MainGrid=build_grid(MainGrid,nwrt);
                LabelGrid=build_grid(LabelGrid,nlabels);
                
                % prepare permutations
                %---------------------
                proto_permutation=cell2mat(mypermutation(1:oo));
...
%             function [newcombos,Container,iG_DerivLocs]=store_combinations(oldcombos)
                ...
                for icol_=1:old_ncols
                    ...
                    for irt=start:nwrt
                        ...
                        extended_locs=locate_permutation(pp,nwrt,wrt_wise);
                        % remove redundancies
