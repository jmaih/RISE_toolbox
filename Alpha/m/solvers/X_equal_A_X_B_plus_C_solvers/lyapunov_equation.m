...
%             V(stable,stable)=real(Us(stable,stable)'*V(stable,stable)*Us(stable,stable));
    ...
    case 'doubling'
        V0=Q;
        G0=T;
        [V,junk,retcode]=fix_point_iterator(@iterator,V0,options);
        if retcode
            retcode=280+retcode;
%             % diagnose this
%             [junk,F0]=iterator(V);
%             stabilized=abs(diag(F0))<1e-9;
%             disp(V(stabilized,stabilized))
        end
    otherwise
...
% function [V,info]=lyapunov_equation(T,Q,MaxIter,TolFun,explosion_limit)
% % solves the equation V=T*V*T'+Q
% if nargin<5
%     explosion_limit=1e+6;
%     if nargin<4
%         TolFun=sqrt(eps);
%         if nargin<3
%             MaxIter=1000;
...
% iter=0;
% info=0;
% conv=0.1*explosion_limit;
% while conv>TolFun && iter<MaxIter && conv<explosion_limit && ~isnan(conv)
%     iter=iter+1;
%     V=V0+G0*V0*G0';
%     conv=max(max(abs(V-V0)));
%     V0=V;
%     G0=G0*G0;
% end
% if iter>=MaxIter
%     info=301;
% elseif any(any(isnan(V))) || isnan(conv)
%     info=302;
% elseif conv>=explosion_limit
%     info=303;
% end
% end
