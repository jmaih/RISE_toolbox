...
options=mysetfield(default_solve,options);
% solving for T
...
while max(conv_T,conv_F)>options.fix_point_TolFun && iter<options.fix_point_maxiter && ...
        conv_F<options.fix_point_explosion_limit && ~any(any(any(isnan(F0))))
    iter=iter+1;
    [T,F0] = iterate_func(T0,varargin{:});
    conv_T=max(max(max(abs(T-T0))));
    conv_F=max(max(max(abs(F0))));
    if fix_point_verbose
        fprintf(1,'%8.0f %12.4f %12.4f\n',iter,conv_T,conv_F); 
    end
    T0=T;
    if any(any(any(isnan(T))))
        break
    end
end
retcode=0;
...
if iter>=options.fix_point_maxiter
    retcode=21;
elseif any(any(isnan(F0)))||any(any(any(isnan(T))))
    retcode=22;
elseif conv_F>=options.fix_point_explosion_limit
    retcode=23;
end

if retcode==0 && any(any(any(isnan(T))))
