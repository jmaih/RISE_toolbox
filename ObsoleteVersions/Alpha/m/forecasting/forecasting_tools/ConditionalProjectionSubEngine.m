function [PbParams,M1,M2,RM2i,OMG,DYbar,UU,SS,Yf,MU,LB,UB,R]=ConditionalProjectionSubEngine(Hypothesis,Nsteps,H,G,Y0,...
...
[R,HHRestr,NumberOfCondShocksPeriods]=BuildShockRestrictions(H,G,yrest_id,xrest_id,ncp,NumberOfAnticipatedPeriods,Hypothesis); %#ok<ASGLU>

% Unconditional forecasts
Yf=InitializeArray(@zeros,endo_nbr,maxiter,1,Y0);
Yf=ComputeForecasts(Yf,H,G,[],maxiter,0);

% Uconditional distribution of endogenous and possibly shocks
% over the forecast horizon:
[DYbar,OMG]=UnconditionalDistributionOfEndogenousAndShocks(R,Yf(yrest_id,1+(1:ncp)));
EndoCond={MUy,OMGy,LBy,UBy,ncv,ncp};
ShocksCond={MUx,OMGx,LBx,UBx,ncvx,ncpx};
[MU,OMG,LB,UB]=ConditionalDistributionOfEndogenousAndShocks(DYbar,OMG,EndoCond,ShocksCond);
...
[MU,OMG,LB,UB,R,DYbar]=RemoveHoles(MU,{OMG},LB,UB,R,DYbar);

[M1,M2,RM2i,qq,UU,SS]=NullAndColumnSpaces(R,verbose);
